...
$('#amka').on('blur', function () {
    useServlet('amka', resetField, 'RegisterServlet', 'field=amka&value=' + $('#amka').val());
});
// Check for duplicate username
$('#username').on('blur', function () {
    useServlet('username', resetField, 'RegisterServlet', 'field=username&value=' + $('#username').val());
});
// Check for duplicate email
$('#email').on('blur', function () {
    useServlet('email', resetField, 'RegisterServlet', 'field=email&value=' + $('#email').val());
});
...
function findLatLon() {
    ...
    var url = "https://forward-reverse-geocoding.p.rapidapi.com/v1/search?q= " + 
                location + 
    ...
    xhr.addEventListener("readystatechange", function () {
      ...
      if($('#lat').get(0) === undefined) {
        $agree.after('<input type="text" id="lat" name="lat" value="' + location[0].lat + '"/>');
        $('#lat').after('<input type="text" id="lon" name="lon" value="' + location[0].lon + '"/>');
        $('#lon').hide();
        $('#lat').hide();
      } else {
